...
int main()
{
	...
	for (int t = 0; t < 10000000; t++)
	{
		...
		if (world.isInside(i, j) == true) // move robot if available
		{
			...
			//world.getCell(i,j).q_    = world.getCell(i, j).reward_ +  world.q_arr2d_[i + world_res_i * j].getMaxQ();
			...
			{ 
				world.getCell(i_old, j_old).q_[1] = world.getCell(i, j).reward_ + world.getCell(i, j).getMaxQ();
			}
			else if (i_old > i )
			{
				world.getCell(i_old, j_old).q_[2] = world.getCell(i, j).reward_ + world.getCell(i, j).getMaxQ();
			}
			else if (i_old < i)
			{
				world.getCell(i_old, j_old).q_[3] = world.getCell(i, j).reward_ + world.getCell(i, j).getMaxQ();
			}
